...
//#define CMSAT
    ...
    inline Lit mkTrueLit(Var vi) { 
        return CMSat::Lit(vi, true);
    }
    ...
    public:
        ...
        bool addClause(Lit x, Lit y, Lit z, Lit zz) {
            vec_lit_t vs;
            vs.push(x);
            vs.push(y);
            vs.push(z);
	...
	Var newVarDump(int wire) { 
            // No need to create vars in lgl, just do our own bookkeeping.
            Var v = numVars;
            numVars += 1;
	    //std::cout << "newvar  " << "introduced_" << wire << "  " << v << std::endl;
            return v;
        }
        ...
        bool addClause(const vec_lit_t& ps) {
			//std::cout << "No. Clauses: " << this->nClauses() << std::endl;
        ...
        */
	    ...
	    std::string y_str = "";
	    if (!y.sign()) {y_str = "-";}
	    std::cout << x_str << x.var() << " " << y_str << y.var() << std::endl;
        */
            return true;
        }
	    ...
	    std::string z_str = "";
	    if (!z.sign()) {z_str = "-";}
	    std::cout << x_str << x.var() << " " << y_str << y.var() << " " << z_str << z.var() << std::endl;
        */
            return true;
        }
	    ...
	    std::string zz_str = "";
	    if (!zz.sign()) {zz_str = "-";}
	    std::cout << x_str << x.var() << " " << y_str << y.var() << " " << z_str << z.var() << " " << zz_str << zz.var() << std::endl;
        */
            return true;
        }
